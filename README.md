...
Place `moretnt-1.2.0+26.2.jar` in the instance's `mods` directory beside Fabric API, then start the game. In Creative mode, all variants appear under the **More TNT** tab. In Survival, every variant is crafted in a normal crafting grid.
...
The `docs/texture-concept-reference.png` sheet was generated as a visual direction reference. The game textures themselves are original, code-generated 64×64 PNG pixel art under `src/main/resources/assets/moretnt/textures/block/`; the reproducible generator is `tools/GenerateTntTextures.java`.
